<template>
	<a-card :bordered="false">
		<a-form ref="searchFormRef" name="advanced_search" :model="searchFormState" class="ant-advanced-search-form">
			<a-row :gutter="24">
				<a-col :span="6">
					<a-form-item label="序列号" name="idxFixedAssetId">
						<a-input disabled v-model:value="route.query.serialNumber" placeholder="请在资产管理选择设备" />
					</a-form-item>
				</a-col>
				<a-col :span="6">
			...
			<template #bodyCell="{ column, record }">
				...
				<template v-if="column.dataIndex === 'devFileThumbnail'">
					...
					<img src="/src/assets/images/fileImg/txt.png" class="record-img"
						v-else-if="record.devFile.suffix.toLowerCase() === 'txt'" />
...
<script setup name="fixedassetfile">
...
// 操作栏通过权限判断是否显示
if (hasPerm(['tFixedAssetFileEdit', 'tFixedAssetFileDelete'])) {
	columns.push({
		title: '操作',
		dataIndex: 'action',
		align: 'center',
...
const loadData = (parameter) => {
	...
	return tFixedAssetFileApi.tFixedAssetFilePage(Object.assign(parameter, searchFormParam)).then((data) => {
		return data
	})
}
// 重置
...
// 删除
const deleteTFixedAssetFile = (record) => {
	let params = [
		{
			pkId: record.pkId
		}
	]
	tFixedAssetFileApi.tFixedAssetFileDelete(params).then(() => {
		table.value.refresh(true)
	})
}
// 批量删除
const deleteBatchTFixedAssetFile = (params) => {
	tFixedAssetFileApi.tFixedAssetFileDelete(params).then(() => {
