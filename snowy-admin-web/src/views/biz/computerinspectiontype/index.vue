		...
		>
			<template #operator class="table-operator">
				<a-space>
					...
					<xn-batch-delete
						v-if="hasPerm('tComputerInspectionTypeBatchDelete')"
			...
			<template #bodyCell="{ column, record }">
				...
				<template v-if="column.dataIndex === 'action'">
					<a-space>
						...
						<a-divider type="vertical" v-if="hasPerm(['tComputerInspectionTypeEdit', 'tComputerInspectionTypeDelete'], 'and')" />
						<a-popconfirm title="确定要删除吗？" @confirm="deleteTComputerInspectionType(record)">
							<a-button type="link" danger size="small" v-if="hasPerm('tComputerInspectionTypeDelete')">删除</a-button>
						</a-popconfirm>
					</a-space>
				</template>
			</template>
