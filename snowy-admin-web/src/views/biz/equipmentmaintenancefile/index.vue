	...
	<a-card :bordered="false">
		...
		>
			<template #operator class="table-operator">
				<a-space>
					<a-button type="primary" @click="formRef.onOpen()" v-if="hasPerm('tEquipmentMaintenanceFileAdd')">
						<template #icon><plus-outlined /></template>
						新增
					</a-button>
					<xn-batch-delete
						v-if="hasPerm('tEquipmentMaintenanceFileBatchDelete')"
						:selectedRowKeys="selectedRowKeys"
						@batchDelete="deleteBatchTEquipmentMaintenanceFile"
					/>
				</a-space>
			</template>
			<template #bodyCell="{ column, record }">
				<template v-if="column.dataIndex === 'action'">
					<a-space>
						<a @click="formRef.onOpen(record)" v-if="hasPerm('tEquipmentMaintenanceFileEdit')">编辑</a>
						<a-divider type="vertical" v-if="hasPerm(['tEquipmentMaintenanceFileEdit', 'tEquipmentMaintenanceFileDelete'], 'and')" />
						<a-popconfirm title="确定要删除吗？" @confirm="deleteTEquipmentMaintenanceFile(record)">
							<a-button type="link" danger size="small" v-if="hasPerm('tEquipmentMaintenanceFileDelete')">删除</a-button>
						</a-popconfirm>
					</a-space>
				</template>
			</template>
