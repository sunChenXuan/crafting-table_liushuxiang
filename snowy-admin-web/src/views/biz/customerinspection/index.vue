			...
			<a-row :gutter="24">
				...
				<a-col :span="6">
					<a-form-item label="巡检类型" name="inspectionType">
						<a-select v-model:value="searchFormState.inspectionType" placeholder="请选择巡检类型" :options="inspectionTypeOptions" />
					</a-form-item>
				</a-col>
				<a-col :span="6">
		...
		>
			<template #operator class="table-operator">
				<a-space>
					...
					<xn-batch-delete
						v-if="hasPerm('tCustomerInspectionBatchDelete')"
			...
			<template #bodyCell="{ column, record }">
				<template v-if="column.dataIndex === 'inspectionType'">
					{{ $TOOL.dictTypeData('GENDER', record.inspectionType) }}
				</template>
				<template v-if="column.dataIndex === 'action'">
					<a-space>
						<a @click="formRef.onOpen(record)" v-if="hasPerm('tCustomerInspectionEdit')">编辑</a>
						<a-divider type="vertical" v-if="hasPerm(['tCustomerInspectionEdit', 'tCustomerInspectionDelete'], 'and')" />
						<a-popconfirm title="确定要删除吗？" @confirm="deleteTCustomerInspection(record)">
							<a-button type="link" danger size="small" v-if="hasPerm('tCustomerInspectionDelete')">删除</a-button>
						</a-popconfirm>
					</a-space>
				</template>
			</template>
...
<script setup name="customerinspection">
	...
	const inspectionTypeOptions = tool.dictList('GENDER')
</script>
