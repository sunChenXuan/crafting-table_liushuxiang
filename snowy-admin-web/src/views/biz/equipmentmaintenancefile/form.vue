<template>
	<xn-form-container
		:title="formData.pkId ? '编辑设备维保文件' : '增加设备维保文件'"
		:width="700"
		:visible="visible"
...
<script setup name="tEquipmentMaintenanceFileForm">
	...
	const onSubmit = () => {
		formRef.value.validate().then(() => {
			...
			tEquipmentMaintenanceFileApi
				.tEquipmentMaintenanceFileSubmitForm(formDataParam, formDataParam.pkId)
				.then(() => {
					onClose()
					emit('successful')
				})
